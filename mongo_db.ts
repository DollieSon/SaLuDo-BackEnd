...
if (!URI) {
  console.error(
    "MONGO_URI environment variable is not set. Database connection will fail.",
  );
  console.error(
    "Set MONGO_URI in your .env file with your MongoDB connection string.",
  );
}
const isLocal =
  URI?.includes("localhost") ||
  URI?.includes("127.0.0.1") ||
  URI?.includes("mongodb:27017");

// Configure client options based on connection type
const clientOptions = isLocal
  ? {} // No TLS for local/docker-compose connections
...
async function createNotificationIndexes(database: Db): Promise<void> {
  try {
    ...
    const preferencesCollection = database.collection(
    ...
    await preferencesCollection.createIndex({
      "emailDigest.enabled": 1,
      "emailDigest.frequency": 1,
    });
